...
$(document).ready(function () {
    ...
    // Only trigger AJAX fetch if there are filters or a non-default page
    var hasFilters = Boolean(search || category || main_category || language || price || level || rating || (page && parseInt(page) !== 1));

    if (hasFilters) {
        // Fetch data on page load (only when needed)
        fetchCourseData(page, filters);
    }
});
