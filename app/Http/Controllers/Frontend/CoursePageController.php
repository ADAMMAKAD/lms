...
class CoursePageController extends Controller
{
    function index() : View {
        ...
        // Load initial courses to ensure the page shows content immediately
        $query = Course::query();
        $query->where(['is_approved' => 'approved', 'status' => 'active']);
        $query->whereHas('category', function($q) {
            $q->where('status', 1);
        });
        $query->with(['instructor:id,name', 'category.translation']);
        $query->orderBy('created_at', 'desc');
        $initialCourses = $query->paginate(9);
        
        return view('frontend.pages.course', compact('categories', 'languages', 'levels', 'initialCourses'));
    }
