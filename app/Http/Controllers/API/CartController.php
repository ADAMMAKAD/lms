...
class CartController extends Controller {
    public function index(): JsonResponse {
        ...
        if ($user->cartCount > 0) {
            $cart_courses = Course::select('id', 'slug', 'title', 'instructor_id', 'thumbnail', 'price', 'discount')->active()->with(['instructor:id,name,image'])
                ->whereHas('carts', fn($q) => $q->where('user_id', $user->id))->withCount(['reviews as average_rating' => fn($q) => $q->select(DB::raw('coalesce(avg(rating), 0)'))->where('status', 1)])->get();
    ...
    public function add_to_cart(string $slug): JsonResponse {
        ...
        // Enrollment check removed - system is now free, users can add any course to cart
