...
class DashboardController extends Controller {
    public function enrolled_courses(Request $request): JsonResponse {
        ...
        // Since system is free, all users have access to all courses
        $courses = Course::select('id', 'instructor_id', 'title', 'slug', 'thumbnail')
            ->with('instructor:id,name,image')
            ->withTrashed()
            // Enrollment count removed - system is now free
            ->orderByDesc('id')
            ->paginate(perPage: $limit);
        
        // Transform to match enrollment structure
        $enrolls = $courses->getCollection()->map(function($course) {
            return (object)['course' => $course, 'course_id' => $course->id];
        });
        $courses->setCollection($enrolls);

        //course percentage calculate
        $courses->getCollection()->transform(function ($enroll) use ($user_id) {
        ...
        if ($courses->isNotEmpty()) {
            $data = EnrolledCourseResource::collection($courses);
    ...
    public function learning_progress(string $slug): JsonResponse {
        ...
        // Since system is free, get course directly by slug
    ...
    public function create_lesson_questions(Request $request, string $course_slug, int $lesson_id): JsonResponse {
        $user = auth()->user();
        $course = Course::select('id')->whereSlug($course_slug)->where('status', 'active')->whereHas('lessons', fn($q) => $q->where('id', $lesson_id))->first();
        if (!$course) {
            return response()->json(['status' => 'error', 'message' => 'Not Found!'], 404);
        }
    ...
    // Order functionality removed - system is now free
    ...
    //         ->with(['user:id,name,email,phone,address'])->first();
    //     if ($order) {
    ...
    /**
     * Check if a user has access to a course (always true in free system).
     *
     * @param $user The user to check.
     * @param string $course_slug The course slug.
     * @return bool Always true since system is now free.
     */
    private function checkEnrollments($user, $course_slug) {
        // In free system, all users have access to all courses
        return Course::where('slug', $course_slug)->where('status', 'active')->exists();
    }
    /**
     * Update course progress.
     *
