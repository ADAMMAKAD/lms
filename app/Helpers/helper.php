...
if (!function_exists('allLanguages')) {
    function allLanguages() {
        ...
        if (!$allLanguages || $allLanguages->isEmpty()) {
            $allLanguages = Language::select('code', 'name', 'direction', 'status')->get();
        }

        return $allLanguages ?? collect();
    }
}
...
if (!function_exists('allCurrencies')) {
    function allCurrencies() {
        ...
        if (!$allCurrencies || $allCurrencies->isEmpty()) {
            $allCurrencies = MultiCurrency::all();
        }

        return $allCurrencies ?? collect();
    }
}
...
if (!function_exists('setEnrollmentIdsInSession')) {
    function setEnrollmentIdsInSession() {
        if (auth('web')->check()) {
            // Since the system is now free, all users have access to all courses
            $enrollmentsIds = \App\Models\Course::pluck('id')->toArray();
...
if (!function_exists('adminSearchRouteList')) {
    function adminSearchRouteList(): object {
        $route_list = [
            ...
            // Order History removed - system is now free
            // Pending Payment removed - system is now free
